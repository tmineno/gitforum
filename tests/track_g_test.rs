...
fn link(git: &GitOps, from: &str, to: &str, rel: &str) {
    evidence_ops::add_thread_link(git, from, to, rel, "human/alice", &fixed_clock()).unwrap();
}
...
fn drive_to_done(git: &GitOps, policy: &Policy, thread_id: &str) {
    ...
    loop {
        ...
        if state_machine::normalize_state_name(&state.status) == "done" {
            break;
        }
        let lifecycle = state.lifecycle();
        let path = state_machine::find_path(lifecycle, &state.status, "done")
