...
- [ ] `Thread`, `Event`, `Evidence`, `Actor`, `Run`, `Approval` の Rust 型
- [ ] issue / rfc / decision の state machine
- [ ] 人間可読 ID 採番
- [ ] `git forum issue new`
- [ ] `git forum rfc new`
- [ ] `git forum decision new`
- [ ] `git forum ls` と kind 別 `ls`
- [ ] `git forum show`
- [ ] `show` 出力の snapshot 方針を固める

Exit criteria:

- [ ] `issue` / `rfc` / `decision` を作成できる
- [ ] `git forum show` で title / kind / state / timeline を表示できる
- [ ] `show` の表示が replay 結果と一致する
- [ ] `show` の snapshot が不安定値なしで比較できる
