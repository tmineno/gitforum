...
fn state_self_loop_returns_zero_with_note() {
    ...
    let git = GitOps::new(repo.path().to_path_buf());
    // task `1v400j3l`: ThreadState dropped its `events: Vec<Event>` field
...
fn state_self_loop_with_comment_records_say_node() {
    ...
    // SPEC-3.0 §2.2 (task `1hg98odf`): comments are nodes in
    // `nodes/<id>.{toml,md}`. The legacy event-chain assertion was on
