...
`ask` (issue), `rfc`, `dec`, and `job` (task).
...
flowchart LR
    ...
    D -- no --> B
    D -- yes --> E[Accept RFC]
    E --> F[Create implementation ask]
    F --> G[Bind branch]
    G --> H[Implement with action / risk / evidence]
    H --> I[Verify and close ask]
```
...
$ git forum node add RFC-a7f3b2x1 --type claim "Need a stable plugin-facing boundary."
$ git forum node add RFC-a7f3b2x1 --type question "What compatibility risks remain?" \
  --as ai/reviewer
$ git forum node add RFC-a7f3b2x1 --type summary \
  "Direction is plausible, but migration evidence is still missing."
$ git forum state RFC-a7f3b2x1 proposed
$ git forum state RFC-a7f3b2x1 under-review
$ git forum state RFC-a7f3b2x1 accepted --approve human/alice
$ git forum new ask "Implement trait backend" \
  --link-to RFC-a7f3b2x1 --rel implements
$ git forum branch bind ASK-m2k9p4n8 feat/trait-backend
$ git forum evidence add ASK-m2k9p4n8 --kind test --ref tests/backend_trait.rs
$ git forum close ASK-m2k9p4n8 --comment "All tests passing."
```
...
New work usually starts as an `rfc`, not an `ask`. An accepted RFC is the decision record. There
...
Asks (issues) are where implementation happens. They can link back to RFCs and bind to Git branches,
so design and code stay connected.
...
- thread: shared abstraction for `ask`, `rfc`, `dec`, and `job` (aliases: `issue`, `task`)
...
- Four thread kinds: `ask`, `rfc`, `dec`, and `job` (aliases: `issue`, `task`), with full state machines
- Opaque content-addressed thread IDs (`KIND-XXXXXXXX`, base36) for conflict-free distributed allocation; legacy sequential IDs remain supported
